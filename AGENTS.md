...
- Native MCP tools (preferred): `search_memories`, `read_memory`, `view_memory`, `write_memory`,
  ...
  claudemon32) — plus `search_code` (structural symbol search over THIS repo's code index;
...
- Search: invoke `search_memories(query: "<text>", project: "claudemon32")` via MCP.
- Record durable facts/decisions as you work with `write_memory` — pass the structure as
  ...
  fixable reason). `project: "claudemon32"`, committed with the repo, visible to every session.
...
- Long-form DOCUMENTS (guides, research, specs) → the wiki tier at write time: `write_memory(project:
  "claudemon32-wiki", …)` for the full prose page, and keep only its distilled facts in the
...
- Search: `search_memories(query: "<text>", project: "claudemon32-wiki")` via MCP, or grep:
  `grep -rn "<query>" wiki/`.
...
- Search: `search_memories(query: "<text>", project: "claudemon32-design")` via MCP, or grep:
...
- Curated overviews: `search_memories(query: "<text>", project: "claudemon32-code")` via MCP, or
  `grep -rn "<query>" code/`.
...
- **Need the actual credential?** Call `get_vendor_credential(vendor: "<slug>", project:
  "claudemon32", reason: "<why>")` instead of asking the user to paste it — Memophant prompts
...
- Search via `search_memories(query: "<text>", project: "claudemon32-vendors")`.
...
- **`/memophant-template <description>`:** find the slug (`search_memories(query, project:
  "claudemon32-templates")` — the templates tier isn't reachable via `list_directory`/
